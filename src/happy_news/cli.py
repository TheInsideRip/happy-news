...
def _write_pages(root: Path, editions_dir: Path, today: date) -> None:
    ...
    today_page = _render_and_validate(edition, is_today=True)
    ...
    for path in sorted(editions_dir.glob("*.json")):
        ...
        day_edition = clock.load_edition(editions_dir, day)
...
def do_run(root: Path, *, dry: bool) -> int:
    ...
    try:
        ...
        # Build and validate before writing/committing anything that reaches
        # the reader. A drought with nothing else published today is quietly
        # skipped inside _write_pages rather than treated as a failure; the
        # edition JSON above still records the empty slot so a later run in
        # this same window can fill it.
